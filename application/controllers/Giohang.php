...
class Giohang extends CI_Controller {
    ...
    public function index(){
        $this->data['title']='Miuxinh - Giỏ hàng của bạn';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            // TODO: insert order detail with cart
            if($this->session->Cart){
                $cart = unserialize($this->session->Cart);

                foreach ($cart->sanPham as $key=>$value){
                    $product = $this->Mproduct->product_detail_id($key);
                    
                    if ($product['number'] >= $value['quanlity']) {
                        
                        $data = array(
                            'orderid' => $orderid,
                            'productid' => $value['info']['id'],
                            'price' => $product['price_sale'] > 0 ? $product['price_sale'] : $product['price'],
                            'count' => $value['quanlity'],
                            'trash' => 1,
                            'status' => 1
                        );
            ...
            $this->data['title']='Miuxinh - Thông tin đơn hàng';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('sessionKhachHang')){
            ...
            $config['smtp_user']    = 'sale.mixinh.shop.2022@gmail.com';
            ...
            $this->email->from('sale.mixinh.shop.2022@gmail.com', 'Miuxinh Store');
            $list = array($val['email']);
            $this->email->to($list);
            $this->email->subject('Cửa hàng Miuxinh');
        ...
        $this->data['title']='Miuxinh Shop.vn - Kết quả đơn hàng';
    ...
    public function coupon(){
        ...
        else
        {
            ...
            $money=0;
            if($this->session->Cart){
                $data=unserialize($this->session->Cart);
                foreach ($data->sanPham as $key => $value) {
                    $row = $this->Mproduct->product_detail_id($key);
                    $total=0;
                    if($row['price_sale'] > 0){
                        $total=(int)$row['price_sale']*(int)$value;
                    }else{
                        $total=(int)$row['price'] * (int)$value;
                    }
                    $money+=$total;
                }
            }
